...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
        ...
        # log the final model WITHOUT using the unsupported "create_logged_model" endpoint
        # Save locally then upload as artifacts (works on DagsHub MLflow tracking)
        with tempfile.TemporaryDirectory() as tmpdir:
            local_model_dir = Path(tmpdir) / "delivery_time_pred_model"
            mlflow.sklearn.save_model(model, path=str(local_model_dir), signature=model_signature)
            mlflow.log_artifacts(str(local_model_dir), artifact_path="delivery_time_pred_model")
